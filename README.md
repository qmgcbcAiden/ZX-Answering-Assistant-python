...
- **学生登录**: 支持账户密码自动登录学生端
- **获取试卷**: 通过 expID 获取考试试卷数据
- **题库加载**: 支持 JSON 格式题库导入和验证
- **答案匹配**: 自动匹配题目ID与题库答案
- **答案注入**: ⚠️ **未完成** - 等待 ExamMemberID 获取方式确定

**当前状态**: 登录、获取试卷、加载题库功能已完成，答案注入功能因缺少关键参数无法完成

**已知限制**:
- 答案注入需要 `ExamMemberID` 参数，但该参数的获取方式尚不明确
- 需要等待网站开放后通过浏览器开发者工具分析确定
- 当前 UI 已实现完整流程，但答案注入按钮暂时无法使用
...
- **cloud_exam_view.py**: 云考试界面（部分实现 - 登录、获取试卷、加载题库已完成，答案注入待ExamMemberID确定）
...
- 需要等待网站开放后通过以下方式分析：
  1. 打开浏览器开发者工具（F12）
  2. 进入云考试页面
  3. 观察 Network 面板的所有API请求
  4. 查找包含 ExamMemberID 的响应
  5. 确定该参数的来源和提取方式
- 确定后需要更新：
  - `src/cloud_exam/workflow.py` - `inject_answers()` 方法
  - `src/cloud_exam/api_client.py` - `submit_answer()` 方法
  - `src/cloud_exam/models.py` - `ExamPaper` 模型
